...
for speciesFile1 in data/*-ensembl.sub.txt
do
    species1=$(basename $speciesFile1)
    species1=${species1//-ensembl.sub.txt/}
    echo $species1
    for speciesFile2 in data/*-ensembl.sub.txt
    do
	...
	if [ $species1 != $species2 ]
	   then
	       (perl parseOMA.2.pl data/$species1\_$species2\_OMA.csv data/$species1-ensembl.sub.txt data/$species2-ensembl.sub.txt > data/$species1\_$species2\_OMA.withSyn.txt) 2>> failedOMAparses.txt
	fi
    done
done
...
#	(perl parseOMA.pl data/$species1\_$species2\_OMA.csv data/$species1.fa data/$species2.fa > data/$species1\_$species2\_OMA.withSyn.txt) 2>> failedOMAparses.txt
